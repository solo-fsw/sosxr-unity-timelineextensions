...
namespace UnityDefaultPlayables.Editor
{
    ...
    ///     For more information see: https://youtu.be/uBPRfcox5hE?si=HouB0bD-MzuY_tFJ&t=1352
    /// </summary>
    public class TimelinePlayableWizard : EditorWindow
    {
        ...
        private readonly GUIContent m_PlayableNameContent = new("Playable Name", "This is the name that will represent the playable.  E.G. TransformTween.  It will be the basis for the class names so it is best not to use the postfixes: 'Clip', 'Behaviour', 'MixerBehaviour' or 'Drawer'.");
        private readonly GUIContent m_StandardBlendPlayableContent = new("Standard Blend Playable", "Often when creating a playable it's intended purpose is just to briefly override the properties of a component for the playable's duration and then blend back to the defaults.  For example a playable that changes the color of a Light but changes it back.  To make a playable with this functionality, check this box.");
        private readonly GUIContent m_TrackBindingTypeContent = new("Track Binding Type", "This is the type of object the Playable will affect.  E.G. To affect the position choose Transform.");
        private readonly GUIContent m_DefaultValuesComponentContent = new("Default Values", "When the scripts are created, each of the selected properties are assigned a default from the selected Component.  If this is left blank no defaults will be used.");
        private readonly GUIContent m_ExposedReferencesContent = new("Exposed References", "Exposed References are references to objects in a scene that your Playable needs. For example, if you want to tween between two Transforms, they will need to be Exposed References.");
        private readonly GUIContent m_BehaviourVariablesContent = new("Behaviour Variables", "Behaviour Variables are all the variables you wish to use in your playable that do NOT need a reference to something in a scene.  For example a float for speed.");
        private readonly GUIContent m_TrackColorContent = new("Track Color", "Timeline tracks have a colored outline, use this to select that color for your track.");
        private readonly GUIContent m_CreateDrawerContent = new("Create Drawer?", "Checking this box will enable the creation of a PropertyDrawer for your playable.  Having this script will make it easier to customise how your playable appears in the inspector.");
        private readonly GUIContent m_StandardBlendPlayablePropertiesContent = new("Standard Blend Playable Properties", "Having already selected a Track Binding type, you can select the properties of the bound component you want the playable to affect.  For example, if your playable is bound to a Transform, you can affect the position property.  Note that changing the component binding will clear the list of properties.");
        private readonly GUIContent m_ClipCapsContent = new("Clip Caps", "Clip Caps are used to change the way Timelines work with your playables.  For example, enabling Blending will mean that your playables can blend when they overlap and have ease in and out durations.  To find out a little about each hover the cursor over the options.  For details, please see the documentation.");
        private readonly GUIContent m_CCNoneContent = new("None", "Your playable supports none of the features below.");
        private readonly GUIContent m_CCLoopingContent = new("Looping", "Your playable has a specified time that it takes and will start again after it finishes until the clip's duration has played.");
        private readonly GUIContent m_CCExtrapolationContent = new("Extrapolation", "Your playable will persist beyond its end time and its results will continue until the next clip is encountered.");
        ...
        */
        ...
        [MenuItem("SOSXR/Timeline/Unity's Playable Wizard...")]
        ...
        {
            var componentTypes = AppDomain.CurrentDomain.GetAssemblies().SelectMany(a => a.GetTypes()).Where(t => typeof(Component).IsAssignableFrom(t)).Where(t => t.IsPublic).ToArray();
        ...
        private void StandardBlendPlayablePropertyGUI(bool findNewProperties)
        {
            if (findNewProperties || (m_TrackBindingProperties == null && m_TrackBindingFields == null))
            {
                ...
                foreach (var trackBindingProperty in m_TrackBindingProperties)
                {
                ...
                foreach (var trackBindingField in m_TrackBindingFields)
                {
        ...
        private string AdditionalNamespacesToString()
        {
            ...
            for (var i = 0; i < exposedReferenceTypes.Length; i++)
            {
                allUsedTypes[i] = exposedReferenceTypes[i];
            }

            for (var i = 0; i < behaviourVariableTypes.Length; i++)
            {
            ...
            for (var i = 0; i < distinctNamespaces.Length; i++)
            {
        ...
        public class UsableProperty : IComparable
        {
            ...
            public void CreateSettingDefaultValueString(Component defaultValuesComponent)
            {
                ...
                var defaultValueObj = usablePropertyType == UsablePropertyType.Property ? propertyInfo.GetValue(defaultValuesComponent, null) : fieldInfo.GetValue(defaultValuesComponent);
